...
- μ_i be the predicted next-period expected return or score,
- Σ be the covariance matrix estimated from trailing returns,
...
The historical-mean model estimates μ as the trailing average daily return over a 252-trading-day lookback window. This is not a sophisticated ML model, but it is an important baseline because it separates the optimizer from forecast-model complexity.
...
3. Top-K by forecast: selects the K assets with the largest μ_i, ignoring covariance.
...
1. estimate μ using only past data,
2. estimate Σ from trailing returns using Ledoit-Wolf shrinkage,
